...
class MarkerTree {
  ...
  search(keyWord:String){
    ...
    for(let i=0;i<this._markerIndexes.length;i++){
      let markerIndex = this._markerIndexes[i];
    
      let displayData = this.getDisplayData(markerIndex);
     
      if(displayData.name.includes(keyWord)){
        console.log('click time',displayData.start);
      }
    }
  }
...
class MarkerTableImpl extends PureComponent<Props> {
  ...
  render() {//绘制标记表
    ...
    const mytree = this.getMarkerTree(
      getMarker,
      markerIndexes,
      zeroAt,
      markerSchemaByName,
