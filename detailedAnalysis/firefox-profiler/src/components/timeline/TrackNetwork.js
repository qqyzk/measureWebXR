...
class NetworkCanvas extends PureComponent<CanvasProps> {
  ...
  drawCanvas(canvas: HTMLCanvasElement) {
    ...
    let totalNetworkStart=null;
    let totalNetworkEnd = null;
    let firstNetwork=true;
    for (let rowIndex = 0; rowIndex < networkTiming.length; rowIndex++) {
      const timing = networkTiming[rowIndex];
      for (let timingIndex = 0; timingIndex < timing.length; timingIndex++) {//便利字典中数据的每一对
        const start =
        ...
        if(firstNetwork){
          totalNetworkStart =  timing.start[timingIndex];
          totalNetworkEnd = timing.end[timingIndex];
          firstNetwork=false;
        }else{
          if( timing.start[timingIndex]<totalNetworkStart){
            totalNetworkStart =  timing.start[timingIndex];
          }
          if( timing.end[timingIndex]>totalNetworkEnd){
            totalNetworkEnd = timing.end[timingIndex];
          }
        }
    ...
    console.log('totalNetworkStart',totalNetworkStart-rangeStart,'totalNetworkEnd',totalNetworkEnd-rangeStart,'Duration',totalNetworkEnd-totalNetworkStart);
