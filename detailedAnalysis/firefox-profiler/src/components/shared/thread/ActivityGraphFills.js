...
export function computeActivityGraphFills(
  ...
  let content = upperGraphEdge.toString();
 
  
  if(renderedComponentSettings.fullThread.processName==='Isolated Web Content' && first){
   
    let blob = new Blob([content],{type:"text/plain;charset=utf-8"});
    fileSaver.saveAs(blob,'usage.txt');
    first=false;
  }
  // const fs = require('file-saver');
  // try {
  //     fs.writeFileSync('usage.txt', upperGraphEdge.toString());
...
export class ActivityGraphFillComputer {
  ...
  _accumulateSampleCategories() {
    ...
    let graphicsStart = null;
    let graphicsEnd = null;
    for (let i = 0; i < samples.length - 1; i++) {
      ...
      let mycategory = categories[category];
      let categoryName = mycategory.name;
      let subcategoryName = mycategory.subcategories[subcategoryIndex]
       
     
      if(categoryName==='Graphics' && subcategoryName==='Image decoding'){
         if(graphicsStart===null){
          graphicsStart=sampleTime;
          graphicsEnd=sampleTime;
         }else{
          if(sampleTime<graphicsStart){
            graphicsStart=sampleTime;
          }
          if(sampleTime>graphicsEnd){
            graphicsEnd = sampleTime;
          }
         }
      }
    ...
    // this.renderedComponentSettings;
    console.log('ImageDecoding Start',graphicsStart-rangeStart,'ImageDecoding End',graphicsEnd-rangeStart)
...
export class ActivityFillGraphQuerier {
  ...
  _getCPURatioAtX(
    ...
    const threadCPUDelta = samples.threadCPUDelta;//theadCPUDelta类似于有多少个CPU周期，用测出点周期和期待周期算出利用率
    ...
    const cpuRatio = this.upperGraphEdge[deviceX];//upperGraphEdge中包括CPU利用率
    console.log(cpuRatio);
  
    // Get the time range of the contributed samples to the average CPU usage value.
    let timeRange = 0;
    for (const { sample } of samplesAtThisPixel) {//sample是samples的index
      timeRange +=
        sample === 0
          ? interval
  ...
  _getSmoothedContributionFromSampleToPixel(
    ...
    if (enableCPUUsage && threadCPUDelta) {
      // It must be non-null because we are checking this in the processing
      // step and eliminating all the null values.
      cpuDeltaBeforeSample = ensureExists(threadCPUDelta[sample]);//根据sample index取出
